...
static char * get_dirname(char * source_path)
{
    char * p = &source_path[strlen(source_path)];

    while((p > source_path) && (*p != '/') && (*p != '\\'))
        p--;
    *p = 0;
...
void get_parentdir(char * source_path, char * target_path)
{
	char * p = &source_path[strlen(source_path)];
	...
	strncpy(target_path, source_path, p - source_path + 1);
...
char *uade_dirname(char *dst, char *src, size_t maxlen)
{
	...
	strlcpy(dst, get_dirname(srctemp), maxlen);
	free(srctemp);
	return dst;
}
...
FILE *uade_open_amiga_file(char *aname, const char *playerdir)
{
    ...
    *real = 0;
    ...
    __android_log_print(ANDROID_LOG_VERBOSE, "UADE", "Looking for %s (%s)", aname, playerdir);
    ...
    if ((separator = strchr(ptr, (int) ':')))
    {
        ...
        if (!(dir = opendir(dirname))) 
		{
            __android_log_print(ANDROID_LOG_VERBOSE, "UADE", "UADE: Can't open dir (%s) (volume parsing)", dirname);
            return 0;
        }
	...
	get_parentdir(aname, parentdirname);
	...
	while ((separator = strchr(ptr, (int) '/')))
	{
		...
		int res = uade_amiga_scandir(real, dirname, fake, sizeof(real));
		if (res > 0)

		{
			...
			{
				__android_log_print(ANDROID_LOG_VERBOSE, "UADE", "UADE: Too long dir path (%s + %s)", dirname, real);
				return 0;
			}
		...
		// check if we have processed all the parts of the path
		else if (strlen(dirname) >= strlen(parentdirname))
		{
			/* didn't find entry */
			return 0;
		}
		...
		else
		{
			...
			strlcat(dirname, fake, sizeof(dirname));
    ...
    if (!(dir = opendir(dirname)))
	{
        __android_log_print(ANDROID_LOG_VERBOSE, "UADE", "Can't open dir (%s) (after dir scanning)", dirname);
        return 0;
    }
