					       ...
					       struct uade_state *state)
{
	...
	*t = 0;
	...
	if (strlen(prefix) < MAX_SUFFIX_LENGTH)
		...
		__android_log_print(ANDROID_LOG_VERBOSE, "UADE", "NAME_CANDIDATE_PREFIX: %s \n", prefix);
	...
	if (namecandidate == NULL) {
		...
		if (strlen(postfix) < MAX_SUFFIX_LENGTH)
			...
			__android_log_print(ANDROID_LOG_VERBOSE, "UADE", "NAME_CANDIDATE_POSTFIX: %s \n", postfix);
	}
	...
	if (ext[0]) {
		...
		if (contentcandidate != NULL) {
			...
			if (namecandidate == NULL ||
				...
				*content = 1;
				__android_log_print(ANDROID_LOG_VERBOSE, "UADE", "CANDIDATE BY CONTENT \n");
	...
	__android_log_print(ANDROID_LOG_VERBOSE, "UADE", "CANDIDATE BY FILENAME \n");
