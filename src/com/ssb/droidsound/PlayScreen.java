...
//	private SongInfo songInfo = new SongInfo();
	...
	public boolean updateHtml()
	{
		...
		try 
		{
			files = Utils.getTemplates(activity);
		} 
		catch (IOException e)
		{
			e.printStackTrace();
		...
		for (String fn : files)
		{
			...
			try
			{
				template_path = "templates/" + fn;
				String html = Utils.readTemplate(activity, template_path);
				String what = fn.substring(0, fn.indexOf(".") );
				templates.put(what.toUpperCase(), html);
			} 
	...
	public static String makeSize(long fileSize) {
		String s;
		s = String.format("%d bytes", fileSize);
		/*
		if(fileSize < 10 * 1024) {
